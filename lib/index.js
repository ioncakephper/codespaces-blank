...
function buildCategory(item, options = {}) {
    let isAutoFolder = isUndefined(item.autofolder) ? false : (isBoolean(item.autofolder) ? item.autofolder : false)
    isAutoFolder = options.parentAutoFolder || isAutoFolder;
    if (isAutoFolder) {
        if (isUndefined(item.path) || isNull(item.path))
        ...
        options = {
            ...options,
            ...{ parentPath: path.join(getSluggedPath(options.parentPath), getSluggedPath(item.path || '')) }
        }

    let isAutoIndex = isUndefined(item.autoindex) ? false : (isBoolean(item.autoindex) ? item.autoindex : false)
    isAutoIndex = options.parentAutoIndex || false || isAutoIndex;

    if (isAutoIndex) {
        options = {
            ...options,
            ...{
                parentAutoFolder: isAutoFolder,
                parentAutoIndex: isAutoIndex,
            }
        }
        let result = {
            type: "category",
    ...
    if (isIndex) {
        ...
        let result = {
            ...
            link: {
                type: "doc",
                id: path.join(getSluggedPath(options.parentPath), getSluggedPath(indexItem.id)).replace(/\\/ig, '/'),
                description: item.description || item.brief
            }
        }
...
function buildHeadings(items, level = 2, options = {}) {
    ...
    return items.map(item => {
        ...
        let isAutoBrief = !isUndefined(item.autobrief) ? false : (isBoolean(item.autobrief) ? item.autobrief : false);
        isAutoBrief = options.parentAutoBrief || isAutoBrief;
    
        options = {...options, ...{parentAutoBrief: isAutoBrief}};
        ...
        return hbsr.render_template('heading', {
            ...
            ...{
                prefix: "#".repeat(level),
                brief: (!item.brief && isAutoBrief) ? getAutoBrief() : item.brief,
...
function buildSidebar(sidebarDef, options) {
    let { items, autogenerated, autoindex, autofolder, autobrief, order } = sidebarDef;
    ...
    options = {
        ...options,
        ...{ 
            parentPath: getSluggedPath(sidebarDef.path || ''),
            parentAutoFolder: isUndefined(autofolder) ? false : (isBoolean(autofolder) ? autofolder : false),
            parentAutoIndex: isUndefined(autoindex) ? false : (isBoolean(autoindex) ? autoindex : false),
            parentAutoBrief: isUndefined(autobrief) ? false : (isBoolean(autobrief) ? autobrief : false)
         }
    }
...
/**
 * The function generates a Lorem Ipsum text with a specified number of sentences.
 * @returns The function `getAutoBrief` returns a string that consists of 5 randomly generated
 * sentences using the Lorem Ipsum text.
 */
function getAutoBrief() {
...
function getSluggedPath(pathString = '') {
    let parts = (isString(pathString)) ? pathString.replace(/\\/ig, '/').trim().split('/') : pathString;
    if (!isArray(parts)) {
