...
const {isArray, isString, isObject, isEmpty} = require('lodash')
...
function buildSidebar(sidebarDef, options) {
    let {items, autogenerated} = sidebarDef; 
    // console.log('Sidebar items');
    // console.log(JSON.stringify(items, null, 4))

    // Generate autogenerated sidebar. Use value of autogenerated property as dirName value.
    if (autogenerated) {
...
`
    let topicContent = hbsr.render(template, {
        ...item,
...
/**
 * The function builds headings for a given list of items with a specified level.
 * @param items - The items parameter is an array of objects representing headings and their
 * subheadings. Each object has properties such as label, brief, and items (which is an array of
 * subheadings).
 * @param [level=2] - The level parameter is a number that specifies the heading level to start with.
 * By default, it is set to 2, which means the function will generate headings starting from level 2
 * (i.e., ##). However, you can pass a different number to start with a different heading level.
 * @returns a string that contains headings formatted with a prefix (determined by the level parameter)
 * and information from the items array passed as an argument. The function recursively calls itself to
 * build nested headings for items that have sub-items.
 */
function buildHeadings(items, level = 2) {

    // console.log("Headings", JSON.stringify(items, null, 4))
    items = items.map(item => normalizeItem(item));
    return items.map(item => {
